...
void CMemView::Refresh(BOOL isdirty)
{
	u8 temp[256];
	int i;
	memRead(temp, (MemRegionType)region, address, 0x100);
	for (i = 0; i < 256; i++)
	{
		if (temp[i] != memory[i])
		{ 
...
LRESULT MemView_ViewBoxPaint(CMemView* wnd, HWND hCtl, WPARAM wParam, LPARAM lParam)
{
	...
	for(line = 0; line < 16; line++, addr += 0x10)
	{
		...
		case 0:
			{
				...
				for(i = 0; i < 16; i++)
				{
					u8 val = T1ReadByte(wnd->memory, ((line << 4) + i));
		...
		case 1:
			{
				...
				for(i = 0; i < 16; i += 2)
				{
					u16 val = T1ReadWord(wnd->memory, ((line << 4) + i));
		...
		case 2:
			{
				...
				for(i = 0; i < 16; i += 4)
				{
					u32 val = T1ReadLong(wnd->memory, ((line << 4) + i));
					if(IsDlgCheckboxChecked(wnd->hWnd,IDC_BIG_ENDIAN))
					{
						char swp[4];
						swp[0] = (val>>24)&0xFF;
		...
		if (wnd->sel && ((wnd->selAddress & 0xFFFFFFF0) == addr))
		{
			...
			{
				u8 val = T1ReadByte(wnd->memory, ((line << 4) + (i + start)));
